...
(function() {
  var Stream, coffee, coffee_helpers, compile, conf, convert, errx, fs, optparse, parse_opts, path, read_file, read_stdin, user_agent;
  ...
  user_agent = function() {
    ...
    return conf.progname + "/" + ver + " (CoffeeScript " + ver_cs + "; " + process.platform + "; " + process.arch + ") node/" + process.versions.node;
  };
  ...
  compile = function(fname, fcontent, opt) {
    var comment, compiled, e, error, key, options, val;
    if (opt == null) {
      opt = {};
    }
    if (fcontent.match(/^\s*$/)) {
