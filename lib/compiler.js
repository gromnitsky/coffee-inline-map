...
(function() {
  ...
  parse_opts = function(src) {
    var opt, p;
    opt = [["-h", "--help", "output usage information & exit"], ["-V", "--version", "output the version number & exit"], ["-o", "--output [FILE]", "write result to a FILE instead of stdout"], ["-l", "--literate", "treat stdin as literate style coffee-script"], ["--no-map", "don't include inline source map (why?)"]];
  ...
  compile = function(fname, fcontent, opt) {
    var comment, compiled, e, inColor, key, options, val;
    if (opt == null) {
      opt = {};
    }
    if (fcontent.match(/^\s*$/)) {
